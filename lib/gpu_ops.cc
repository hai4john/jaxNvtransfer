...
PYBIND11_MODULE(gpu_ops, m) {
    ...
    m.def("build_nvtransfer_descriptor",
        [](std::int64_t send_buf_size, std::int64_t recv_buf_size, int src_rank, int dst_rank) {
            return PackDescriptor(nvtransferDescriptor{send_buf_size, recv_buf_size, src_rank, dst_rank});
        }
    );
}
